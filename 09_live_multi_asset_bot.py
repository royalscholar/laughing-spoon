...
# -------------------------------
ib = IB()
ib.connect('127.0.0.1', 7497, clientId=20)  # Paper account
...
# Stop loss / take profit (in pips / USD)
STOP_LOSS = 0.002
TAKE_PROFIT = 0.004
...
def execute_order(contract, signal, price):
    ...
    if signal in ["BUY", "SELL"]:
        ...
        with open('trade_log.csv', 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([contract.symbol, signal, qty, price, time.time()])
    else:
...
while True:
    ...
    ib.sleep(60)  # Run every 1 minute
